...
# Load matched tracks
matched_tracks_path = Path(f"output/{sequence}_multicamera_tracks.json")
try:
...
# Find the first frame where all cameras have detections
first_frame = 1
all_cameras_have_detections = False

while not all_cameras_have_detections and first_frame < 1000:  # Limit search to first 1000 frames
    all_cameras_have_detections = True
    for camera_id in video_captures.keys():
        if camera_id not in detection_results or first_frame not in detection_results[camera_id]:
            all_cameras_have_detections = False
            break
    
    if not all_cameras_have_detections:
        first_frame += 1

print(f"Starting visualization from frame {first_frame}")

# Set videos to the first frame
...
while running:
    ...
    for camera_id, frame in frames.items():
        ...
        cv2.putText(frame, f"Camera {camera_id} - Frame {frame_idx}", 
    ...
    # Create a canvas for side-by-side visualization
    if frames:
        # Get maximum dimensions
        max_height = max(frame.shape[0] for frame in frames.values())
        max_width = max(frame.shape[1] for frame in frames.values())
        
        # Create canvas for side-by-side display
        num_cameras = len(frames)
        canvas_width = max_width * num_cameras
        ...
        for i, (camera_id, frame) in enumerate(frames.items()):
            h, w = frame.shape[:2]
            canvas[:h, i * max_width:i * max_width + w] = frame
        
        # Show the combined visualization
        cv2.imshow("Multi-Camera Tracking", canvas)
