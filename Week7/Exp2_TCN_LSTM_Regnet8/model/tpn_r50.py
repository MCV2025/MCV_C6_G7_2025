...
class TPN_R50(nn.Module):
    ...
    def forward(self, x):
        ...
        frame_features = []
        for t in range(T):
            feats_t = [f[:, t, ...] for f in aggregated_feats]  # each is (B, C, H, W)
            fused_t = self.neck(feats_t)  # (B, neck_out_channels, H_fused, W_fused)
            # Global average pool spatially.
            pooled_t = F.adaptive_avg_pool2d(fused_t, (1, 1)).view(B, -1)  # (B, neck_out_channels)
            frame_features.append(pooled_t)
        # Stack along time: (B, T, neck_out_channels)
