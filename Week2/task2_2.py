...
class ObjectTracker:
    def __init__(self, iou_threshold=0.5):
        self.tracked_objects = {}  # Stores {ID: (KalmanFilter, bbox, frame_last_seen)}
        self.next_id = 1
    ...
    def update(self, detections, frame_idx):
        ...
        for det in detections:
            ...
            for obj_id, (kalman, prev_bbox, last_frame) in self.tracked_objects.items():
                predicted = kalman.predict()
                pred_x, pred_y = predicted[0], predicted[1]  # predicted position
            ...
            if best_id is not None:
                # Use the Kalman filter to correct the prediction with the new detection
                best_kalman.correct(np.array([det['bbox'][0] + det['bbox'][2] / 2,  # x
                                              det['bbox'][1] + det['bbox'][3] / 2], dtype=np.float32))
                updated_tracks[best_id] = (best_kalman, det['bbox'], frame_idx)
                assigned_ids.add(best_id)
            else:
                kalman = self._create_kalman_filter()
                # Use the detection to initialize the Kalman filter
                kalman.correct(np.array([det['bbox'][0] + det['bbox'][2] / 2,  # x
                                         det['bbox'][1] + det['bbox'][3] / 2], dtype=np.float32))
                updated_tracks[self.next_id] = (kalman, det['bbox'], frame_idx)
                assigned_ids.add(self.next_id)
                self.next_id += 1
        
        # Remove old objects if not seen in recent frames
        self.tracked_objects = {
            obj_id: (kalman, bbox, last_frame)
            for obj_id, (kalman, bbox, last_frame) in updated_tracks.items()
...
def save_detections_mots(output_txt, tracked_objects, frame_idx):
    with open(output_txt, "a") as f:
        for obj_id, (kalman, bbox, _) in tracked_objects.items():
            x1, y1, x2, y2 = bbox
            width = x2 - x1
            height = y2 - y1
...
def process_video(video_path, output_path, annotation_file, output_txt):
    ...
    while cap.isOpened():
        ...
        # Correct unpacking of tracked_objects dictionary
        for obj_id, (kalman, bbox, _) in tracked_objects.items():
